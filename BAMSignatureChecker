function Is-OSFile {
    ...
    # Common OS file paths or directories
    $osPaths = @(
        "C:\Windows\",
        "C:\Windows\System32\",
        "C:\Windows\SysWOW64\"
    ...
    # Check if the file path starts with any of the OS paths
    foreach ($osPath in $osPaths) {
        if ($FilePath -like "$osPath*") {
            return $true
        }
    }
    return $false
...
Write-Host -ForegroundColor Blue "   ╚═╝  ╚═╝╚═╝   ╚═╝   ╚══════╝   ╚═╝       ╚═════╝ ╚═╝  ╚═╝╚═╝     ╚═╝       ╚═╝    ╚═════╝  ╚═════╝ ╚══════╝";
Write-Host "";
Write-Host -ForegroundColor Red "   Made By: RitzySix";
Write-Host -ForegroundColor Red "   Details: Signature Checker and EXE Finder";
Write-Host -ForegroundColor Red "   discord.gg/x3BUaPk369"; -NoNewLine
Write-Host "";                                                                                                           
            ...
            If($key.length -eq 24){
                ...
                [PSCustomObject]@{
						    ...
						    'Last Execution User Time' = $TimeUser
						     ...
						     Path =  		$path
                             Signature =          $Sig
                             'OS' = if ($path) { Is-OSFile -FilePath $path } else { $false }  # Check if the file is an OS file
						     User =         $User
						     SID =          $Sid
                             Regpath =        $rp
                             }}}}}

$Bam | Out-GridView -PassThru -Title "BAM key entries $($Bam.count)  - User TimeZone: ($UserTime) -> Ritzy Bam Tool"
